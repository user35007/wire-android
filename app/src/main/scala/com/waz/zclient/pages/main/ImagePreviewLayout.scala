...
import com.waz.zclient.pages.main.ImagePreviewLayout.Camera
...
class ImagePreviewLayout(context: Context, attrs: AttributeSet, style: Int) extends FrameLayout(context, attrs, style) with ViewHelper with ConfirmationMenuListener {
  ...
  private lazy val imageView = returning(findViewById[ImageView](R.id.iv__conversation__preview)) { view =>
    view.onClick {
      if (approveImageSelectionMenu.isVisible) {
        if (sketchShouldBeVisible.currentValue.getOrElse(false)) ViewUtils.fadeOutView(sketchMenuContainer)
        ViewUtils.fadeOutView(approveImageSelectionMenu)
        if (!TextUtils.isEmpty(titleTextView.getText)) ViewUtils.fadeOutView(titleTextViewContainer)
      } else {
        if (sketchShouldBeVisible.currentValue.getOrElse(false)) ViewUtils.fadeInView(sketchMenuContainer)
        ViewUtils.fadeInView(approveImageSelectionMenu)
        if (!TextUtils.isEmpty(titleTextView.getText)) ViewUtils.fadeInView(titleTextViewContainer)
      }
    }
  }
  ...
  private lazy val titleTextViewContainer = returning(findViewById[FrameLayout](R.id.ttv__image_preview__title__container)) { container =>
    convName.map(TextUtils.isEmpty).onUi(empty => container.setVisible(!empty))
  }
  ...
  def setImage(imageData: Array[Byte], isMirrored: Boolean): Unit = {
    this.source = Option(Camera)
    this.imageAsset = Option(ImageAssetFactory.getImageAsset(imageData)) // probably wrong
  ...
  def setImage(uri: URI, source: ImagePreviewLayout.Source): Unit = {
    ...
    this.imageAsset = Option(ImageAssetFactory.getImageAsset(uri)) // probably wrong
    imageView.setImageDrawable(ImageAssetDrawable(uri))
  }
...
object ImagePreviewLayout {
  ...
  def CAMERA(): ImagePreviewLayout.Source = Camera // for java
