...
object ImageAssetDrawable {
  ...
  def apply(uri: URI)(implicit inj: Injector, eventContext: EventContext): ImageAssetDrawable = {
    val asset = AssetData.newImageAssetFromUri(tag = Medium, uri = uri)
    new ImageAssetDrawable(Signal.const(DataImage(asset)), scaleType = ScaleType.CenterCrop)
  }
